...
__all__ = ['resnet20_fp', 'resnet20_quant_allQ']
...
class QuantBlock_frozen(nn.Module):
    expansion = 1
    def __init__(self, in_planes, planes, args, stride=1, option='A'):
        super(QuantBlock_frozen, self).__init__()
...
class ResNet_allQ(nn.Module):
    def __init__(self, block, num_blocks, args, num_classes=10):
        ...
        self.conv1 = QConv_frozen_first(3, 16, kernel_size=3, stride=1, padding=1, bias=False, args=args)
        self.bn1 = nn.BatchNorm2d(16)
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2)
        self.bn2 = nn.BatchNorm1d(64)
        self.linear = QLinear(64, num_classes, args, bias=True)
...
def resnet20_quant_allQ(args):
    return ResNet_allQ(QuantBlock_frozen, [3, 3, 3], args)
